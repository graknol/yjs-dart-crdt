// YText implementation with Y.js optimization - single operations for consecutive text
// Based on Y.js performance optimization for consecutive typing/pasting

import '../polyfill.dart' hide createID, compareIDs;
import '../structs/Item_fixed.dart';
import '../utils/ID.dart' show createID, compareIDs;

/// Content type for text operations (optimized for blocks, not characters)  
class ContentString {
  final String str;
...
/// Optimized YText CRDT implementation matching Y.js performance
class YTextOptimized {
  dynamic _start;
  int _length = 0;
  ...
  YTextOptimized([String? initialText]) {
  ...
  /// Insert text at given index using Y.js optimization
  void insert(int index, String text, [Map<String, dynamic>? attributes]) {
    ...
    if (doc != null) {
      _transact(() {
        ...
        _insertTextOptimized(pos, text, attributes ?? {});
      });
    } else {
  ...
  /// Insert text using Y.js optimization (single Item per text block)
  void _insertTextOptimized(ItemTextListPosition pos, String text, Map<String, dynamic> attributes) {
    final clientId = _getClientId();
    final clock = _getNextClock();
    
    // Y.js OPTIMIZATION: Create single Item for entire text block
    ...
    // Create single Item with entire text content
    final item = Item(
      id: textId,
      left: left,
  ...
  void _integrateItem(Item item) {
    ...
    _length += item.content.getLength();
  }
